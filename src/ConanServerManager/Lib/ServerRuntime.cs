...
namespace ServerManagerTool.Lib
{
    public class ServerRuntime : DependencyObject, IDisposable
    {
        ...
        public string GetServerExe() => Path.Combine(this.ProfileSnapshot.InstallDirectory, Config.Default.ServerBinaryRelativePath, Config.Default.ServerExeFile);
        ...
        private void ProcessStatusUpdate(IAsyncDisposable registration, ServerStatusUpdate update)
        {
            ...
            TaskUtils.RunOnUIThreadAsync(() =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(serverExeFile) && File.Exists(serverExeFile))
                {
                    try
                    {
                        var info = FileVersionInfo.GetVersionInfo(serverExeFile);
        ...
        public async Task<bool> UpgradeAsync(CancellationToken cancellationToken, bool updateServer, BranchSnapshot branch, bool validate, bool updateMods, string[] updateModIds, ProgressDelegate progressCallback)
        {
            ...
            {
                var result = MessageBox.Show("The server requires a 64-bit operating system to run. Your operating system is 32-bit and therefore the server manager will be unable to start the server, but you may still install it or load and save profiles and settings files for use on other machines.\r\n\r\nDo you wish to continue?", "64-bit OS Required", MessageBoxButton.YesNo, MessageBoxImage.Warning);
                if (result == MessageBoxResult.No)
                {
                    return false;
                }
            ...
            try
            {
                        ...
                        if (modDetails != null)
                        {
                            ...
                            for (var index = 0; index < modIdList.Count; index++)
                            {
                                ...
                                if (downloadMod)
                                {
                                    ...
                                    {
                                        progressCallback?.Invoke(0, $"{SteamCmdUpdater.OUTPUT_PREFIX} Forcing mod download - Server Manager setting is TRUE.");
                                    }
                                    else if (modDetail == null)
                                    {
                                        if (forceUpdateMods)
                                        {
                                            progressCallback?.Invoke(0, $"{SteamCmdUpdater.OUTPUT_PREFIX} Forcing mod download - Mod details not available and Server Manager setting is TRUE.");
                                        }
                                        else
                                        {
                            ...
                            ModUtils.CreateModListFile(this.ProfileSnapshot.InstallDirectory, this.ProfileSnapshot.ServerModIds);
                            progressCallback?.Invoke(0, "Modlist file updated.");
                ...
                {
                    if (updateServer && updateMods && modIdList.Count > 0)
                    {
                        progressCallback?.Invoke(0, $"{SteamCmdUpdater.OUTPUT_PREFIX} ***********************************************************");
